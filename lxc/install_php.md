...
case "$php_version" in
    8.1)
        packagelist="{cgi,cli,fpm,bcmath,common,ctype,curl,exif,fileinfo,gd,gmp,imagick,imap,intl,ldap,mbstring,mysql,mysqlnd,opcache,pdo,pgsql,readline,soap,tidy,tokenizer,xml,xmlrpc,zip}"
        ;;
    7.1)
        packagelist="{cgi,cli,fpm,bcmath,common,ctype,curl,exif,fileinfo,gd,gmp,imagick,imap,intl,json,ldap,mbstring,mcrypt,mysql,mysqlnd,opcache,pdo,pgsql,readline,soap,tidy,tokenizer,xml,xmlrpc,zip}"
        ;;
    *)
        packagelist="{cgi,cli,fpm,bcmath,common,ctype,curl,exif,fileinfo,gd,gmp,imagick,imap,intl,json,ldap,mbstring,mysql,mysqlnd,opcache,pdo,pgsql,readline,soap,tidy,tokenizer,xml,xmlrpc,zip}"
        ;;
esac
if sudo apt-get -y install "php$php_version-$packagelist" >/dev/null && sleep 1; then
    echo "Installed PHP $php_version packages"
else
    fail "sudo apt -y install [php packages...]"
fi
```
